...
<h3 class="title">Daftar History Interview</h3>
        ...
        <tbody>
            @foreach ($apply_loker as $index => $applok)
                ...
                <tr>
                    ...
                    <td>{{ $lok->gaji_max }}</td>
                    <td>
                        @foreach ($tahapan_apply as $tahapan)
                            @if ($tahapan->idapply == $applok->idapply && $tahapan->idtahapan == 2)
                                {{ $tahapan->tgl_update }}
                            @endif
                        @endforeach
                    </td>
                    <td>
                        @foreach ($tahapan_apply as $tahapan)
                            @if ($tahapan->idapply == $applok->idapply && $tahapan->idtahapan == 2)
                                @if ($tahapan->idnilai == 1)
                                    Lolos
                                @elseif ($tahapan->idnilai == 0)
                                    Gagal
                                @endif
                            @endif
                        @endforeach
